"""Log Monitor Agent - Main workflow definition with OpenTelemetry tracing.
...
OpenTelemetry tracing captures the full execution for observability and evaluation.
"""
...
_tracer = otel_trace.get_tracer("log-monitor-agent")
...
def classify_log(state: AgentState) -> dict:
    ...
    """
    with _tracer.start_as_current_span("classify_log") as span:
        log_message = state["log_message"]
        span.set_attribute("input.log_message", log_message[:200])
        ...
        if not log_message or not log_message.strip():
            ...
            span.set_attribute("output.classification", "normal")
...
Provide your classification with confidence score and the indicators you found."""
        ...
        span.set_attribute("output.classification", result.classification)
...
def diagnose_problem(state: AgentState) -> dict:
    ...
    """
    with _tracer.start_as_current_span("diagnose_problem") as span:
        from .tools import get_mcp_tools, get_research_tool_guidance

        log_message = state["log_message"]
        classification = state["classification"]
        span.set_attribute("input.classification", classification)
...
Keep the diagnosis concise (1-2 sentences)."""
        ...
        span.set_attribute("output.diagnosis", diagnosis[:2000])
...
def assess_severity(state: AgentState) -> dict:
    ...
    """
    with _tracer.start_as_current_span("assess_severity") as span:
        diagnosis = state["diagnosis"]
        log_message = state["log_message"]
        span.set_attribute("input.diagnosis", diagnosis[:500])
...
If uncertain, default to LOW severity."""
        ...
        try:
            ...
            span.set_attribute("output.severity", severity)
            span.set_attribute("output.reasoning", result.reasoning[:500])
        except Exception:
...
def alert_sre(state: AgentState) -> dict:
    ...
    """
    with _tracer.start_as_current_span("alert_sre") as span:
        ...
        span.set_attribute("output.action_taken", "slack_alert")
...
def manage_github_ticket(state: AgentState) -> dict:
    ...
    Returns:
        Updated state with action_taken field.
    """
    with _tracer.start_as_current_span("manage_github_ticket") as span:
        ...
        span.set_attribute("output.action_taken", "github_ticket")
...
def set_no_action(state: AgentState) -> dict:
    ...
    Returns:
        Updated state with action_taken field.
    """
    with _tracer.start_as_current_span("set_no_action") as span:
        span.set_attribute("output.action_taken", "none")
...
def process_log_message(log_message: str) -> dict:
    ...
    Example:
        ...
        ...
    """
    with _tracer.start_as_current_span("process_log_message") as root:
        root.set_attribute("input.log_message", log_message[:200])

        with _tracer.start_as_current_span("initialize_state") as span:
            ...
            span.set_attribute("output.state_keys", str(list(initial_state.keys())))

        with _tracer.start_as_current_span("workflow_execution") as span:
            span.set_attribute("input.log_message_preview", log_message[:100])
            workflow = get_workflow()
            final_state = workflow.invoke(initial_state)
            span.set_attribute("output.classification", final_state.get("classification", ""))
            span.set_attribute("output.severity", final_state.get("severity", ""))
            span.set_attribute("output.action_taken", final_state.get("action_taken", ""))

        root.set_attribute("output.classification", final_state.get("classification", ""))
        root.set_attribute("output.severity", final_state.get("severity", ""))
        root.set_attribute("output.action_taken", final_state.get("action_taken", ""))
        root.set_attribute("output.diagnosis", final_state.get("diagnosis", "")[:2000])
